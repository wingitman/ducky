...
func defaultThemeFile() string {
	...
	if err != nil {
		...
		if herr != nil {
			return filepath.Join(".config", "delbysoft", "themes.toml")
		}
		return filepath.Join(home, ".config", "delbysoft", "themes.toml")
	}
	return filepath.Join(base, "delbysoft", "themes.toml")
}
...
// SetThemeName updates only the selected theme in the ducky config file.
